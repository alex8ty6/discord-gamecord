  ...
  async approve() {
    return new Promise(async resolve => {
      ...
      collector.on('end', async (_, reason) => {
        ...
        // Author: Alex Wanjohi
        if (reason === 'timeout') {
          this.emit('gameTimeOut', { result: reason, player: this.message.author, opponent: this.opponent });
        ...
        if (reason === 'reject') {
          // Author: Alex Wanjohi
          this.emit('gameReject', { result: reason, player: this.message.author, opponent: this.opponent });
          return resolve(false);
        }
